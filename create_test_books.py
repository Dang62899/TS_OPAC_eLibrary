...
def create_test_books():
    ...
    author, _ = Author.objects.get_or_create(
        first_name="Test", last_name="Author", defaults={"bio": "Author for testing purposes"}
    )

    subject, _ = Subject.objects.get_or_create(name="Testing")

    pub_type, _ = PublicationType.objects.get_or_create(
        name="Book", defaults={"code": "BK", "description": "Standard book format"}
    )

    location, _ = Location.objects.get_or_create(
        code="MAIN", defaults={"name": "Main Library", "description": "Main library location"}
    )

    # Get a test borrower (create if needed)
    borrower, created = User.objects.get_or_create(
        username="testuser",
        defaults={"email": "testuser@library.com", "first_name": "Test", "last_name": "User", "user_type": "student"},
    )
    if created:
    ...
    if created:
        ...
        for i in range(1, 3):
            Item.objects.create(publication=pub1, barcode=f"AVAIL-{i:03d}", location=location, status="available")
    ...
    if created:
        ...
        # Available item
        Item.objects.create(publication=pub2, barcode="PARTIAL-001", location=location, status="available")
        # On loan item
        item_on_loan = Item.objects.create(publication=pub2, barcode="PARTIAL-002", location=location, status="on_loan")
    ...
    if created:
        ...
        for i in range(1, 4):
            item = Item.objects.create(
                publication=pub3, barcode=f"UNAVAIL-{i:03d}", location=location, status="on_loan"
            )
            Loan.objects.create(
                item=item,
                borrower=borrower,
    ...
    if created:
        ...
        # 1 available
        Item.objects.create(publication=pub4, barcode="POPULAR-001", location=location, status="available")
        # 4 on loan
        for i in range(2, 6):
            item = Item.objects.create(
                publication=pub4, barcode=f"POPULAR-{i:03d}", location=location, status="on_loan"
            )
            Loan.objects.create(
                item=item,
                borrower=borrower,
    ...
    if created:
        ...
        item = Item.objects.create(publication=pub5, barcode="SINGLE-001", location=location, status="on_loan")
        Loan.objects.create(
            item=item,
            borrower=borrower,
