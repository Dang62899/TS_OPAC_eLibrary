...
def my_account(request):
    ...
    ).select_related('item__publication').order_by('-return_date')[:10]
    ...
    checkout_requests = CheckoutRequest.objects.filter(
        borrower=request.user,
        status__in=['pending', 'approved']
    ).select_related('publication').order_by('-request_date')
