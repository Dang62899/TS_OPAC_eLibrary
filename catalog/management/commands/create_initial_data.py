    ...
    def handle(self, *args, **options):
        ...
        for name, code, desc, is_phys in locations_data:
            location, created = Location.objects.get_or_create(
                code=code, defaults={"name": name, "description": desc, "is_physical": is_phys}
            )
            if created:
        ...
        publisher, created = Publisher.objects.get_or_create(
            ...
            defaults={"address": "123 Library Street, Book City, BC 12345", "website": "https://example.com"},
        )
        if created:
        ...
        if not Publication.objects.filter(title="System Administration Manual").exists():
            ...
            for i in range(1, 4):
                Item.objects.create(
                    publication=pub, barcode=f"MAN001-{i:03d}", location=main_location, status="available"
                )
        ...
        if not Publication.objects.filter(title="Emergency Response Procedures").exists():
            ...
            for i in range(1, 3):
                Item.objects.create(
                    publication=pub, barcode=f"SOP005-{i:03d}", location=main_location, status="available"
                )
