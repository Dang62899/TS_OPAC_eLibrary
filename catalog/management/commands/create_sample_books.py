...
class Command(BaseCommand):
    ...
    def create_manuals(self):
        ...
        pub_type, _ = PublicationType.objects.get_or_create(
            code="MAN", defaults={"name": "Manual", "description": "Technical and operational manuals"}
        )

        publisher, _ = Publisher.objects.get_or_create(
            name="Technical Publications Inc.", defaults={"website": "https://techpub.example.com"}
        )

        location, _ = Location.objects.get_or_create(
            code="MAIN", defaults={"name": "Main Library", "description": "Main collection", "is_physical": True}
        )
        ...
        for idx, manual_data in enumerate(manuals, 1):
            ...
            author2, _ = Author.objects.get_or_create(
                first_name="Jane", last_name=f"Expert{idx}", defaults={"bio": "Senior systems engineer and consultant"}
            )
            ...
            pub, created = Publication.objects.get_or_create(
                ...
                defaults={
                    ...
                    "edition": (
                        f"{idx}st Edition"
                        if idx == 1
                        else f"{idx}nd Edition" if idx == 2 else f"{idx}rd Edition" if idx == 3 else f"{idx}th Edition"
                    ),
                    "language": "English",
    ...
    def create_sops(self):
        ...
        pub_type, _ = PublicationType.objects.get_or_create(
            code="SOP", defaults={"name": "SOP", "description": "Standard Operating Procedures"}
        )

        publisher, _ = Publisher.objects.get_or_create(
            name="Standards & Procedures Publishing", defaults={"website": "https://soppub.example.com"}
        )
    ...
    def create_capstone_projects(self):
        ...
        pub_type, _ = PublicationType.objects.get_or_create(
            code="CAP", defaults={"name": "Capstone Project", "description": "Student capstone and thesis projects"}
        )

        publisher, _ = Publisher.objects.get_or_create(
            name="Academic Research Archives", defaults={"website": "https://research.example.edu"}
        )

        location, _ = Location.objects.get_or_create(
            code="ARCH", defaults={"name": "Archives", "description": "Academic archives", "is_physical": True}
        )
    ...
    def create_ttps(self):
        ...
        pub_type, _ = PublicationType.objects.get_or_create(
            code="TTP", defaults={"name": "TTP", "description": "Tactics, Techniques, and Procedures"}
        )

        publisher, _ = Publisher.objects.get_or_create(
            name="Security Research Institute", defaults={"website": "https://securityresearch.example.org"}
        )
