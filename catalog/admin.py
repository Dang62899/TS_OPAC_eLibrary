...
class PublicationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'subtitle', 'authors', 'publication_type')
        }),
        ('Publication Details', {
            'fields': ('publisher', 'publication_date', 'edition', 'isbn', 'language', 'pages')
        }),
        ('Classification', {
            'fields': ('subjects', 'call_number')
        }),
        ('Description', {
            'fields': ('abstract', 'summary', 'cover_image')
        }),
    )
